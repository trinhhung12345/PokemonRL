...
class StreamWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.steam_step_counter >= self.upload_interval:
            if self.env.use_screen_explore:
                # Nếu dùng khám phá màn hình, hiển thị số lượng frame trong KNN
                explore_info = f"frames: {self.env.knn_index.get_current_count()}"
            else:
                # Nếu dùng khám phá tọa độ, hiển thị số lượng tọa độ
                explore_info = f"coords: {len(self.env.seen_coords)}"

            self.stream_metadata["extra"] = explore_info
            self.loop.run_until_complete(
